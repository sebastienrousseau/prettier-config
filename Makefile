...
# POSIX-compatible Makefile for prettier-config.
...
lint:
	npm install --no-save --no-audit --no-fund eslint@10.10.0 prettier@3.6.2 @sebastienrousseau/eslint-config
	npx eslint --no-config-lookup --config node_modules/@sebastienrousseau/eslint-config/index.cjs --max-warnings 0 $(LINT_PATHS)
	npx prettier --config ./index.cjs --check $(LINT_PATHS)

format:
	npm install --no-save --no-audit --no-fund prettier@3.6.2
	npx prettier --config ./index.cjs --write $(LINT_PATHS)
